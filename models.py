...
class BIN_Interaction_Flat(nn.Sequential):
    ...
    def __init__(self, **config):
        ...
        self.dsim_size = config['dsim_size']
        self.psim_size = config['psim_size']
        ...
        self.dcnn = nn.Conv1d(50, 3, 3, padding=0)
        self.pcnn = nn.Conv1d(545, 3, 3, padding=0)

        self.d_sim_cnn = nn.Sequential(
            nn.Conv1d(68, 64, 1, padding=0),
            nn.ReLU(),
            nn.Conv1d(64, 32, 1, padding=0),
            nn.ReLU()
        )

       #self.d_sim_cnn = nn.Conv1d(68, 32, 1, padding=0)

        self.p_sim_cnn = nn.Sequential(
            nn.Conv1d(379, 128, 1, padding=0),
            nn.ReLU(),
    ...
    def forward(self, d, p, d_mask, p_mask, d_sim, p_sim):
        ...
        #print(d_encoded_layers.shape)   torch.Size([16, 50, 384])
        p_encoded_layers = self.p_encoder(p_emb.float(), ex_p_mask.float())
        #print(p_encoded_layers.shape)   torch.Size([16, 545, 384])

        drug = d_encoded_layers.view(int(self.batch_size / self.gpus), 50, -1)
        protein = p_encoded_layers.view(int(self.batch_size / self.gpus), 545, -1)
        drug = F.dropout(drug, p=self.dropout_rate)
        protein = F.dropout(protein, p=self.dropout_rate)

        drug = self.dcnn(drug)
        protein = self.pcnn(protein)
        #print(i_1.shape)   torch.Size([16, 3, 382])
        # print("d_sim.shape: ", d_sim.shape)
        # print("p_sim.shape: ", p_sim.shape)
        d_sim = d_sim.view(int(self.batch_size / self.gpus), 68, -1)
        p_sim = p_sim.view(int(self.batch_size / self.gpus), 379, -1)
        # print("d_sim_view.shape: ", d_sim.shape)
        # print("p_sim_view.shape: ", p_sim.shape)
        d_sim = F.dropout(d_sim, p=self.dropout_rate)
        p_sim = F.dropout(p_sim, p=self.dropout_rate)

        d_sim = self.d_sim_cnn(d_sim)
        p_sim = self.p_sim_cnn(p_sim)

        #i_2 = torch.cat([d_sim, p_sim], 2)

        i_1 = drug * protein  # interaction
        i_2 = d_sim * p_sim
       # i_2 = torch.cat([d_sim, p_sim], 2)  # interaction
       #  print("i_1.shape: ", i_1.shape)
       #  print("i_2.shape:", i_2.shape)

        f1 = i_1.view(int(self.batch_size / self.gpus), -1)
        f2 = i_2.view(int(self.batch_size / self.gpus), -1)
